...
function getTips(nodes, conns, ch) {
  if (!nodes.length) return [{ sev:"error", icon:"📋", title:"Canvas is empty",
    msg:"Start by dragging shapes from the left panel. Begin with a Start terminal, then add the shapes needed for this problem." }];
  ...
  const tips = [];
  if ((cnt.terminal||0)<2)
    tips.push({ sev:"error", icon:"🔴", title:"Missing Start/End terminals",
      msg:`Every flowchart must have a START and END terminal (rounded rectangles). You have ${cnt.terminal||0}. Drag ${2-(cnt.terminal||0)} more from the palette.` });
  ...
  if (isolated.length)
    tips.push({ sev:"error", icon:"🔗",
      ...
      msg:"Switch to 🔗 Connect mode, click the source shape first (it glows orange), then click the destination. Every shape must be part of the flow." });

  nodes.filter(n=>n.type==="decision").forEach(d=>{
    if ((out[d.id]||0)<2)
      tips.push({ sev:"tip", icon:"💡",
        title:`"${d.text}" needs 2 outgoing arrows`,
        msg:`A Decision diamond must have TWO arrows out — one for YES and one for NO. It currently has ${out[d.id]||0}.` });
  });

  if (conns.length===0&&nodes.length>1)
    tips.push({ sev:"tip", icon:"➡️", title:"No arrows drawn yet",
      msg:"Switch to 🔗 Connect mode in the toolbar, then click shapes in sequence to draw arrows between them." });

  if (!tips.length)
    tips.push({ sev:"ok", icon:"✅", title:"Looking good!",
...
async function loadUsers() {
  try { const r=localStorage.getItem("fb_users"); return r?JSON.parse(r):[]; } catch{ return []; }
}
async function saveUsers(u) {
  try { localStorage.setItem("fb_users",JSON.stringify(u)); } catch{}
}
async function loadCustomEx() {
  try { const r=localStorage.getItem("fb_exercises"); return r?JSON.parse(r):{jss2:[],ss2:[]}; } catch{ return {jss2:[],ss2:[]}; }
}
async function saveCustomEx(e) {
  try { localStorage.setItem("fb_exercises",JSON.stringify(e)); } catch{}
}
...
function PaletteItem({ type, onDragStart }) {
  ...
  return <div draggable onDragStart={e=>onDragStart(e,type)}
    onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
    style={{marginBottom:7,cursor:"grab",opacity:hov?1:.8,transform:hov?"translateX(3px)":"none",transition:"all .15s"}}>
    <Shape type={type} text={SYM[type].label} w={W(type)} h={H(type)}/></div>;
}
...
function Landing({ onStudent, onTeacher }) {
  return <div style={{height:"100vh",background:"#060d1a",display:"flex",flexDirection:"column",
    ...
    <div style={{textAlign:"center",marginBottom:44}}>
      ...
      <h1 style={{fontSize:30,fontWeight:900,color:"#38bdf8",margin:0,letterSpacing:-1}}>FlowBuilder</h1>
      <p style={{color:"#2a4a70",fontSize:13,marginTop:7}}>Interactive Flowchart Practice · Computer Studies</p>
    </div>
    <div style={{display:"flex",gap:16}}>
      <button onClick={onStudent} style={{padding:"14px 34px",borderRadius:12,border:"none",cursor:"pointer",
        background:"#0c4a6e",color:"#7dd3fc",fontSize:14,fontWeight:800,fontFamily:"inherit",
        boxShadow:"0 0 24px #0c4a6e55"}}>👨‍🎓 I'm a Student</button>
      <button onClick={onTeacher} style={{padding:"14px 34px",borderRadius:12,
        border:"1.5px solid #1e3a5f",cursor:"pointer",background:"transparent",
        color:"#475569",fontSize:14,fontWeight:800,fontFamily:"inherit"}}>👩‍🏫 Teacher Login</button>
    </div>
    <p style={{color:"#1e3a5f",fontSize:10,marginTop:28}}>Teacher password: <strong style={{color:"#334d6e"}}>teacher2024</strong></p>
  </div>;
}

// ── Student Auth ──────────────────────────────────────────────────────────────
function StudentAuth({ users, onLogin, onRegister, onBack }) {
  ...
  const inp={width:"100%",padding:"9px 12px",borderRadius:7,border:`1px solid ${BORDER}`,
    background:"#0a1628",color:"#e2e8f0",fontSize:13,fontFamily:"inherit",outline:"none"};

  const go = () => {
    const t=name.trim();
    if (!t) { setErr("Please enter your name."); return; }
    if (tab==="login") {
      const u=users.find(u=>u.name.toLowerCase()===t.toLowerCase()&&u.level===level);
      if (!u) { setErr("Not found. Check name & class, or register first."); return; }
      onLogin(u);
    } else {
      if (users.find(u=>u.name.toLowerCase()===t.toLowerCase()&&u.level===level))
        { setErr("Name already exists in this class. Try logging in."); return; }
      onRegister(t,level);
    }
  };
  ...
  return <div style={{height:"100vh",background:BG,display:"flex",alignItems:"center",
    ...
    <div style={{background:PANEL,borderRadius:14,border:`1px solid ${BORDER}`,padding:"30px 34px",width:340}}>
      ...
      <h2 style={{margin:"0 0 18px",fontSize:19,fontWeight:800,color:"#38bdf8"}}>Student Portal</h2>
      <div style={{display:"flex",marginBottom:18,background:"#0a1628",borderRadius:8,padding:3}}>
      ...
      {err&&<div style={{fontSize:11,color:"#f87171",marginBottom:12,background:"#1a0505",
        padding:"6px 10px",borderRadius:6}}>{err}</div>}
      <button onClick={go} style={{width:"100%",padding:"10px",borderRadius:8,border:"none",cursor:"pointer",
        background:"#0284c7",color:"#fff",fontSize:13,fontWeight:800,fontFamily:"inherit"}}>
...
function TeacherAuth({ onLogin, onBack }) {
  ...
  const inp={width:"100%",padding:"9px 12px",borderRadius:7,border:`1px solid ${BORDER}`,
    background:"#0a1628",color:"#e2e8f0",fontSize:13,fontFamily:"inherit",outline:"none"};
  const go=()=>{ if(pass===TEACHER_PASS) onLogin(); else setErr("Incorrect password."); };
  return <div style={{height:"100vh",background:BG,display:"flex",alignItems:"center",
    justifyContent:"center",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
    <div style={{background:PANEL,borderRadius:14,border:`1px solid ${BORDER}`,padding:"30px 34px",width:320}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:"#475569",cursor:"pointer",
        fontSize:12,marginBottom:14,padding:0,fontFamily:"inherit"}}>← Back</button>
      <h2 style={{margin:"0 0 18px",fontSize:19,fontWeight:800,color:"#38bdf8"}}>Teacher Login</h2>
      <div style={{marginBottom:16}}>
        <label style={{fontSize:11,color:"#475569",display:"block",marginBottom:4}}>Password</label>
        <input type="password" value={pass} onChange={e=>setPass(e.target.value)}
          placeholder="Enter teacher password" style={inp} onKeyDown={e=>e.key==="Enter"&&go()}/>
      </div>
      {err&&<div style={{fontSize:11,color:"#f87171",marginBottom:12}}>{err}</div>}
      <button onClick={go} style={{width:"100%",padding:"10px",borderRadius:8,border:"none",cursor:"pointer",
...
// ── Teacher Dashboard ─────────────────────────────────────────────────────────
function TeacherDashboard({ users, customEx, baseEx, onAdd, onDelete, onLogout }) {
  ...
  const inp={width:"100%",padding:"7px 10px",borderRadius:6,border:`1px solid ${BORDER}`,
    background:"#0a1628",color:"#e2e8f0",fontSize:12,fontFamily:"inherit",outline:"none"};
  ...
  const submit=()=>{
    ...
    onAdd(addLvl,{title:form.title.trim(),desc:form.desc.trim(),hint:form.hint.trim(),steps:form.steps,req});
  ...
  return <div style={{height:"100vh",background:BG,color:"#e2e8f0",
    ...
    <header style={{background:PANEL,borderBottom:`1px solid ${BORDER}`,height:48,
      ...
      <span style={{fontSize:15,fontWeight:800,color:"#38bdf8"}}>FlowBuilder</span>
      <span style={{fontSize:10,color:MUTED,background:"#0c2540",padding:"2px 8px",borderRadius:10,fontWeight:700}}>Teacher</span>
      <div style={{display:"flex",gap:4,marginLeft:8}}>
        <TabBtn id="students" label="👥 Students"/>
        <TabBtn id="exercises" label="📚 Exercises"/>
        <TabBtn id="add" label="➕ Add Exercise"/>
      </div>
      <button onClick={onLogout} style={{marginLeft:"auto",padding:"3px 12px",borderRadius:5,
    ...
    <div style={{flex:1,overflow:"auto",padding:"18px 22px"}}>
      ...
      {tab==="students"&&<div>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
          <h2 style={{margin:0,fontSize:15,fontWeight:800,color:"#f1f5f9"}}>Students ({filtered.length})</h2>
          <div style={{display:"flex",gap:5}}>
            {["all","jss2","ss2"].map(l=><button key={l} onClick={()=>setFilterLvl(l)}
              style={{padding:"2px 11px",borderRadius:10,border:`1px solid ${filterLvl===l?"#38bdf8":BORDER}`,
        ...
        {filtered.length===0
          ...
          : <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))",gap:10}}>
            {filtered.map(u=>{
              const p=getProg(u);
              const exs=allEx(u.level);
              return <div key={u.id} style={{background:PANEL,borderRadius:10,border:`1px solid ${BORDER}`,padding:"13px 15px"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:9}}>
                  <div style={{width:30,height:30,borderRadius:"50%",background:"#0c4a6e",display:"flex",
                    alignItems:"center",justifyContent:"center",fontSize:13,color:"#7dd3fc",fontWeight:800,flexShrink:0}}>
                    {u.name[0].toUpperCase()}</div>
                  <div><div style={{fontSize:13,fontWeight:700,color:"#f1f5f9"}}>{u.name}</div>
                    <div style={{fontSize:10,color:MUTED,fontWeight:600}}>{u.level.toUpperCase()}</div></div>
                  <div style={{marginLeft:"auto",fontSize:13,fontWeight:800,
                    color:p.pct===100?"#4ade80":p.pct>50?"#fbbf24":"#60a5fa"}}>{p.pct}%</div>
                </div>
                <div style={{height:5,background:"#1e3a5f",borderRadius:3,overflow:"hidden",marginBottom:7}}>
                ...
                <div style={{fontSize:10,color:MUTED,marginBottom:8}}>{p.done} / {p.total} completed</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                  {exs.map(ex=>{
                    ...
                    return <div key={ex.id} title={`${ex.title}${att?" · "+att+" attempt(s)":""}`}
                      style={{fontSize:9,padding:"2px 6px",borderRadius:6,fontWeight:700,
                        background:done?"#052e16":"#0a1628",color:done?"#4ade80":"#1e3a5f",
                        border:`1px solid ${done?"#16a34a":"#172333"}`}}>
                      {done?"✓ ":""}{ex.title.length>12?ex.title.slice(0,10)+"…":ex.title}</div>;
                  })}
                </div>
              </div>;
            })}
      ...
      {/* Exercises tab */}
      {tab==="exercises"&&<div>
        <h2 style={{margin:"0 0 14px",fontSize:15,fontWeight:800,color:"#f1f5f9"}}>All Exercises</h2>
        {["jss2","ss2"].map(lvl=><div key={lvl} style={{marginBottom:22}}>
          <h3 style={{margin:"0 0 8px",fontSize:12,fontWeight:800,color:MUTED,textTransform:"uppercase",letterSpacing:1}}>{lvl.toUpperCase()}</h3>
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            {allEx(lvl).map((ex,i)=>{
              const isCustom=!baseEx[lvl].find(b=>b.id===ex.id);
              return <div key={ex.id} style={{display:"flex",alignItems:"center",gap:10,
                background:PANEL,borderRadius:8,padding:"9px 13px",border:`1px solid ${BORDER}`}}>
                <span style={{fontSize:10,color:MUTED,minWidth:18}}>{i+1}.</span>
                <div style={{flex:1}}>
                  <div style={{fontSize:12,fontWeight:700,color:"#f1f5f9"}}>{ex.title}</div>
                  <div style={{fontSize:11,color:"#475569"}}>{ex.desc}</div></div>
                {isCustom&&<>
                  <span style={{fontSize:9,background:"#1e1b4b",color:"#a5b4fc",padding:"2px 6px",borderRadius:6,fontWeight:700}}>Custom</span>
                  <button onClick={()=>onDelete(lvl,ex.id)} style={{padding:"2px 8px",borderRadius:5,border:"none",
                    cursor:"pointer",background:"#3f0707",color:"#f87171",fontSize:11,fontWeight:700,fontFamily:"inherit"}}>Delete</button>
                </>}
              </div>;
            })}
          </div>
      ...
      {/* Add exercise tab */}
      {tab==="add"&&<div style={{maxWidth:580}}>
        <h2 style={{margin:"0 0 14px",fontSize:15,fontWeight:800,color:"#f1f5f9"}}>Add New Exercise</h2>
        <div style={{display:"flex",gap:7,marginBottom:14}}>
          {["jss2","ss2"].map(l=><button key={l} onClick={()=>setAddLvl(l)}
            style={{padding:"5px 18px",borderRadius:7,border:`1.5px solid ${addLvl===l?"#38bdf8":BORDER}`,
        ...
        <div style={{marginBottom:12}}>
          <label style={{fontSize:11,color:"#475569",display:"block",marginBottom:6}}>Solution Steps * (add in order)</label>
          <div style={{display:"flex",gap:5,marginBottom:5}}>
            ...
            <input value={newStep.text} onChange={e=>setNewStep(s=>({...s,text:e.target.value}))}
              placeholder="Step label" style={{...inp,flex:1}} onKeyDown={e=>e.key==="Enter"&&addStep()}/>
            <button onClick={addStep} style={{padding:"7px 12px",borderRadius:6,border:"none",cursor:"pointer",
              background:"#0369a1",color:"#fff",fontSize:12,fontWeight:700,fontFamily:"inherit"}}>+ Add</button>
          </div>
          {form.steps.length>0&&<div style={{background:"#0a1628",borderRadius:8,padding:"7px 9px",
            border:`1px solid ${BORDER}`,maxHeight:170,overflowY:"auto"}}>
            {form.steps.map((s,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:7,padding:"3px 0",
        ...
        {msg==="error"&&<div style={{fontSize:11,color:"#f87171",marginBottom:10,background:"#1a0505",padding:"6px 10px",borderRadius:6}}>Fill in Title, Description and add at least 2 steps.</div>}
        {msg==="ok"&&<div style={{fontSize:11,color:"#4ade80",marginBottom:10,background:"#052e16",padding:"6px 10px",borderRadius:6}}>✓ Exercise added!</div>}
        <button onClick={submit} style={{padding:"9px 22px",borderRadius:8,border:"none",cursor:"pointer",
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      const [u,ex]=await Promise.all([loadUsers(),loadCustomEx()]);
      setUsers(u); setCustomEx(ex); setLoaded(true);
    })();
  },[]);
  ...
  const pushHist=useCallback((nn,nc)=>{
    setHistory(h=>{
      ...
      return [...base,{nodes:nn,conns:nc}].slice(-MAX_HIST);
    });
  ...
  const saveProgress=useCallback((userId,exId,isComplete)=>{
    setUsers(prev=>{
      const updated=prev.map(u=>{
        if(u.id!==userId) return u;
        const ex=u.progress?.[exId]||{};
        const progress={...u.progress,[exId]:{
          ...ex,
          attempts:(ex.attempts||0)+1,
          lastAttempt:Date.now(),
          ...(isComplete?{completed:true,completedAt:Date.now()}:{})
        }};
        return {...u,progress};
      });
      saveUsers(updated);
  ...
  const canvasMouseUp=useCallback(()=>{
    if(dragNode.current&&didMove.current){
      setNodes(ns=>{
        const snapped=ns.map(n=>n.id===dragNode.current.id?{...n,x:snap(n.x),y:snap(n.y)}:n);
        pushHist(snapped,conns);
        return snapped;
      });
    }
  ...
  },[mode,cfrom,conns,nodes,selNode,pushHist]);
  ...
  const checkFlowchart=useCallback(()=>{
    ...
    const isOk=t[0].sev==="ok"||t.every(x=>x.sev!=="error");
    saveProgress(currentUser.id,ch.id,isOk);
    setFeedback(isOk
      ?{ok:true,msg:"✓ Excellent! Exercise marked as complete. See Solution to compare your layout."}
  ...
  if(screen==="studentAuth") return <StudentAuth users={users}
    onLogin={u=>{setCurrentUser(u);setCIdx(0);setScreen("student");}}
    onRegister={(name,level)=>{
      const u={id:`u${Date.now()}`,name,level,createdAt:Date.now(),progress:{}};
      const updated=[...users,u]; setUsers(updated); saveUsers(updated);
      setCurrentUser(u); setCIdx(0); setScreen("student");
    }}
    onBack={()=>setScreen("landing")}/>;

  if(screen==="teacherAuth") return <TeacherAuth onLogin={()=>setScreen("teacher")} onBack={()=>setScreen("landing")}/>;

  if(screen==="teacher") return <TeacherDashboard users={users} customEx={customEx} baseEx={BASE}
    onAdd={(lvl,ex)=>{
      const updated={...customEx,[lvl]:[...(customEx[lvl]||[]),{...ex,id:`c${Date.now()}`}]};
      setCustomEx(updated); saveCustomEx(updated);
    }}
    onDelete={(lvl,id)=>{
      const updated={...customEx,[lvl]:customEx[lvl].filter(e=>e.id!==id)};
      setCustomEx(updated); saveCustomEx(updated);
    }}
    ...
    return <div style={{display:"flex",flexDirection:"column",height:"100vh",background:BG,
      ...
      <header style={{background:PANEL,borderBottom:`1px solid ${BORDER}`,height:48,
        ...
        <svg width="18" height="18" viewBox="0 0 22 22" fill="none">
          <rect x="1" y="6" width="20" height="10" rx="2" fill="#0ea5e9" opacity=".2" stroke="#38bdf8" strokeWidth="1.4"/>
          <circle cx="11" cy="11" r="2" fill="#38bdf8"/>
          <path d="M11 2v4M11 16v4M2 11h4M16 11h4" stroke="#38bdf8" strokeWidth="1.4" strokeLinecap="round"/>
        </svg>
        ...
        <span style={{fontSize:10,color:MUTED,background:"#0c3554",padding:"2px 8px",borderRadius:10,fontWeight:700}}>
          {currentUser.level.toUpperCase()}</span>
        {/* Progress bar */}
        <div style={{display:"flex",alignItems:"center",gap:6,marginLeft:4}}>
          <span style={{fontSize:10,color:MUTED,fontWeight:600}}>Progress:</span>
          <div style={{width:90,height:5,background:"#1e3a5f",borderRadius:3,overflow:"hidden"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:6,alignItems:"center"}}>
          <span style={{fontSize:9,color:MUTED}}>Double-click to rename · Ctrl+Z undo · Ctrl+Y redo</span>
          <button onClick={()=>{setCurrentUser(null);setScreen("landing");}}
            style={{padding:"3px 10px",borderRadius:5,border:`1px solid ${BORDER}`,background:"transparent",
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        {/* Sidebar */}
        <aside style={{width:182,background:PANEL,borderRight:`1px solid ${BORDER}`,
          display:"flex",flexDirection:"column",overflow:"hidden",flexShrink:0}}>
          <div style={{padding:"10px 10px 8px",borderBottom:`1px solid ${BORDER}`}}>
            <div style={{fontSize:9,fontWeight:800,color:MUTED,textTransform:"uppercase",letterSpacing:1.2,marginBottom:7}}>
              Symbols — drag to canvas</div>
            {SYM_ORDER.map(type=><PaletteItem key={type} type={type} onDragStart={palDragStart}/>)}
          </div>
          <div style={{flex:1,overflowY:"auto",padding:"8px 7px"}}>
            <div style={{fontSize:9,fontWeight:800,color:MUTED,textTransform:"uppercase",letterSpacing:1.2,marginBottom:7,padding:"0 3px"}}>
            ...
            {exList.map((c,i)=>{
              ...
              return <button key={c.id} onClick={()=>setCIdx(i)}
                style={{display:"flex",alignItems:"center",gap:6,width:"100%",textAlign:"left",
                  padding:"7px 9px",borderRadius:6,border:`1px solid ${cIdx===i?"#1d5f8a":"#172333"}`,
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          {/* Challenge info */}
          <div style={{background:"#09141f",padding:"7px 14px",borderBottom:`1px solid ${BORDER}`,flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}>
              <span style={{fontWeight:800,fontSize:13,color:"#f1f5f9"}}>{ch.title}</span>
              {progress[ch.id]?.completed&&<span style={{fontSize:10,background:"#052e16",color:"#4ade80",
                padding:"1px 8px",borderRadius:10,fontWeight:700}}>✓ Completed</span>}
            </div>
          ...
          {/* Toolbar */}
          <div style={{background:BG,padding:"5px 11px",display:"flex",gap:4,alignItems:"center",
            ...
            {[["move","✋","Move"],["connect","🔗","Connect"],["delete","✕","Delete"]].map(([m,icon,lbl])=>(
              <button key={m} onClick={()=>{setMode(m);setCfrom(null);}}
                style={{padding:"3px 9px",borderRadius:5,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:"inherit",
            ...
            <button onClick={undo} disabled={histIdx<=0} title="Ctrl+Z"
              style={{padding:"3px 8px",borderRadius:5,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,
                fontFamily:"inherit",background:"#172333",color:histIdx<=0?"#2a3a4a":"#7dd3fc",opacity:histIdx<=0?.4:1}}>↩ Undo</button>
            <button onClick={redo} disabled={histIdx>=history.length-1} title="Ctrl+Y"
              style={{padding:"3px 8px",borderRadius:5,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,
                fontFamily:"inherit",background:"#172333",color:histIdx>=history.length-1?"#2a3a4a":"#7dd3fc",
                opacity:histIdx>=history.length-1?.4:1}}>↪ Redo</button>
            <div style={{width:1,height:16,background:BORDER,margin:"0 1px"}}/>
            <button onClick={()=>{setNodes([]);setConns([]);setHistory([{nodes:[],conns:[]}]);setHistIdx(0);setFeedback(null);setShowTips(false);}}
              style={{padding:"3px 8px",borderRadius:5,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:"inherit",background:"#172333",color:"#4a6280"}}>🧹 Clear</button>
            <button onClick={checkFlowchart}
              style={{padding:"3px 11px",borderRadius:5,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:"inherit",background:"#15803d",color:"#fff"}}>✓ Check</button>
            {mode==="connect"&&<span style={{fontSize:10,color:"#fbbf24",fontWeight:700,marginLeft:4}}>
              {cfrom?"→ Click destination":"→ Click source first"}</span>}
            {mode==="delete"&&<span style={{fontSize:10,color:"#f87171",fontWeight:700,marginLeft:4}}>→ Click shape or × on arrow</span>}
          </div>

          {/* Feedback */}
          {feedback&&<div style={{padding:"6px 13px",flexShrink:0,fontSize:11,fontWeight:600,display:"flex",gap:8,
            ...
            <button onClick={()=>setFeedback(null)} style={{background:"none",border:"none",color:"inherit",cursor:"pointer",fontSize:15}}>×</button>
          </div>}

          {/* Correction tips */}
          {showTips&&tips.length>0&&<div style={{padding:"8px 13px",flexShrink:0,background:"#070f1e",
            borderBottom:`1px solid ${BORDER}`,maxHeight:155,overflowY:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
              ...
              <button onClick={()=>setShowTips(false)} style={{background:"none",border:"none",color:MUTED,cursor:"pointer",fontSize:13}}>×</button>
            </div>
          ...
          <div style={{flex:1,display:"flex",overflow:"hidden"}}>
            <div style={{flex:1,overflow:"auto"}}>
              <div ref={canvasRef}
                ...
                onClick={()=>{if(mode==="connect")setCfrom(null);else setSelNode(null);}}
                onMouseMove={canvasMouseMove} onMouseUp={canvasMouseUp} onMouseLeave={canvasMouseUp}>
                ...
                {mode==="delete"&&conns.map(c=>{const mid=getMid(c.from,c.to);return mid?
                  <button key={`dx-${c.from}-${c.to}`} onClick={()=>deleteConn(c.from,c.to)}
                    style={{position:"absolute",left:mid.x-9,top:mid.y-9,width:18,height:18,borderRadius:"50%",
                      border:"none",background:"#dc2626",color:"#fff",fontSize:11,fontWeight:800,
                      cursor:"pointer",zIndex:5,lineHeight:"18px",textAlign:"center",padding:0}}>×</button>:null;})}
                ...
                {nodes.map(nd=><div key={nd.id}
                  style={{position:"absolute",left:nd.x,top:nd.y,width:W(nd.type),height:H(nd.type),zIndex:2,
                    ...
                    filter:(selNode===nd.id||cfrom===nd.id)?`drop-shadow(0 0 7px ${cfrom===nd.id?"#f97316":"#0ea5e9"})`:"none",
                ...
                {nodes.length===0&&<div style={{position:"absolute",inset:0,display:"flex",
                  ...
                  <div style={{textAlign:"center",color:"#1a3050"}}>
                    <div style={{fontSize:40,opacity:.5,marginBottom:10}}>📋</div>
                    <div style={{fontSize:14,fontWeight:700,marginBottom:4}}>Drag shapes from the left panel</div>
                    <div style={{fontSize:11}}>Then use Connect mode to draw arrows between them</div>
                  </div>
                </div>}
              </div>
            </div>
